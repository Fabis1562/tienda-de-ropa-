...
export function ProductCard({ name, price, image, onAddToCart, onViewDetails }: ProductCardProps) {
  return (
    <div 
      className="bg-red rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all cursor-pointer group border border-gray-100"
      onClick={onViewDetails}
    >
      <div className="aspect-[3/4] bg-red-100 relative overflow-hidden">
        <ImageWithFallback 
          src={image} 
          alt={name}
        ...
        />
        <button 
          className="absolute top-3 right-3 w-9 h-9 bg-red/90 backdrop-blur-sm rounded-full flex items-center justify-center shadow-md hover:bg-red "
          onClick={(e) => {
            e.stopPropagation();
          }}
        >
